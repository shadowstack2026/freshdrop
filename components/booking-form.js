...
export default function BookingForm({ embedded = false, onSubmitSuccess }) {
  ...
  return (
    <div className={`${embedded ? "" : "container py-10 max-w-3xl"}`}>
      {!embedded && (
        <>
      ...
      <div className={`grid gap-6 ${embedded ? "" : "md:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]"}`}>
        <Card className={`${embedded ? "border-none shadow-none" : ""}`}>
        ...
        <div className="space-y-4">
          <Card className={`${embedded ? "bg-white border-none shadow-sm" : ""}`}>
