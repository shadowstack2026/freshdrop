    ...
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur-md">
      ...
      {menuOpen && (
        <>
          ...
          />
          <div
            className="fixed top-0 right-0 z-50 h-full w-[80vw] max-w-[360px] bg-white px-6 py-6 shadow-2xl transition-transform duration-300"
            ...
            <div className="mt-6 space-y-3 text-base">
              {(isLoggedIn ? loggedInNavLinks : publicNavLinks).map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 font-semibold text-slate-900 transition hover:border-primary hover:text-primary"
                  onClick={() => setMenuOpen(false)}
                >
                  {link.label}
                </Link>
